...
if ( file_exists( dirname(__FILE__).'/replaceVars.plugin.php' ) ) {
	// load LESS plugin
	require_once( dirname(__FILE__).'/replaceVars.plugin.php' );
}
...
class wp_less {
	...
	public function parse_stylesheet( $src, $handle ) {
		...
		if(is_multisite()) {
			$handle .= '-blog' . get_current_blog_id();
		}
	...
	public function get_cached_file_data( $path ) {
		...
		if ( isset( $caches[$path] ) ) {
			$entry     = $caches[ $path ];
			$less_path = str_replace( WP_CONTENT_URL, WP_CONTENT_DIR, $entry['url'] );

			if ( ! file_exists( $less_path ) || ! is_readable( $less_path ) ) {
				return null;
			}
	...
	public function save_parsed_css( $css_path, $file_contents ) {
		...
		if ( ! is_readable( $css_path ) || ! is_writable( $css_path ) ) {
			@touch( $css_path );
			@chmod( $css_path, 0755 );
		}
	...
	public function get_cache_dir( $path = true ) {
		...
		if ( $path ) {
			...
			$dir = apply_filters( 'wp_less_cache_url', path_join( $upload_dir[ 'basedir' ], 'wp-less-cache' ) );
