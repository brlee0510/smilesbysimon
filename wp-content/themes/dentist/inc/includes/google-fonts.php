 ...
 *
 * @source https://github.com/vafour/vafpress-framework/
 */
class VP_Site_GoogleWebFont
{
	...
	{
		$gwf_file = get_template_directory() . '/public/gwf.json';
		if(!file_exists($gwf_file)) {
			return;
		}

		$gwf_file = @file_get_contents($gwf_file);
		if($gwf_file) {
			$gwf_file = @json_decode($gwf_file, true);
		}

		if($gwf_file) {
			$this->_google_fonts = $gwf_file;
		}
	}
	...
	public function add($name, $weights = 'normal', $styles = 'normal', $subsets = null)
	{
		...
		if ( empty( $name ) || ( ! empty( $this->_google_fonts ) && ! isset( $this->_google_fonts[ $name ] ) ) ) {
			return;
		}
